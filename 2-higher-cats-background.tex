...
\comment{Note to self: woke up last night (Mon 8 Nov) having realised there was a serious typo somewhere, in a displayed forula defining something I think; now can't remember where it was!  To do: look carefully for it!}
...
This is made precise by Mark Weber in \cite{weber:monoidal-pseudo-algebras}: $\MonGlobCat$ as defined here is (2-equivalent to) the 2-category of normalised pseudo-algebras for the ``internal strict $\omega$-category'' 2-monad $\Tcal$ on $[\G,\Cat]$.

\begin{figure}$\begin{array}{ccc}
\bfig
\node Cn(300,-100)[A_n]
...
\arrow|r|[B1r`B0r;t]
\efig
\end{array}$\caption{\label{fig:some-spans} A higher span; an identity span.}
\end{figure}
...
Identity spans are defined as in Fig.~\ref{fig:some-spans}; composition $\tensor_k$ of spans is defined by pullback, as in Fig.~\ref{fig:composite-spans}.  \todo{[Do this big fun diag!]}

A functor $\C \to \C'$ preserving pullbacks induces a monoidal globular functor $\Spans[\C] \to \Spans[C']$. 
\end{example}

We will often apply this in the case where $\C = \D^\op$, for some familiar $\D$; so then we will work with \emph{cospans} in $\D$, whose composites are computed by pushouts, and so on.

Although of course technically identical, there is an important difference in intuition between the two orientations.  When working in $\Spans[\C]$, we think of the objects of a span as objects \emph{containing} cells, such as the sets of $n$-cells of a higher category.  When working $\Spans[\D^\op]$, we think of the objects of a span as objects \emph{representing} cells, such as the topological globes $D^n$ or the universal globes $\yon(n) \in \GSets$, with ``co-source'' and ``co-target'' maps $s,t \colon A^{s,t}_i \to A_{i+1}$ embedding the lower-dimensional globes as the sources or targets of higher ones. \\
...
Since $\Lan_\yon \Cbu$ preserves colimits, it lifts to give a monoidal globular functor $\Spans[\Lan_\yon \Cbu] \colon \Spans[\GSets^\op] \to \Spans[\C^\op]$, and since it sends the basic globes $\yon$ to $\Cbu$, we have moreover a commutative diagram:
$$\bfig
\node pd(0,0)[\pd]
...
We then call $\A$ an \emph{internal $P$-algebra} in $\E$, or when $\E = \Spans[\C]$, an internal $P$-algebra in $\C$.
...
and hence a $P$-algebra structure on the globular set $\Hom(X,\A)$. 
...
Roughly, this operation removes the ``leftmost'' cell of dimension $> 1$ if there are any such, or of dimension $1$ if there are none higher.  It is clear by induction on e.g.\ dimension that under repeated application of $(-)^-$, any pasting diagram eventually reaches $()$.
\end{para}

\begin{para}\todo{Give needed statements re the realisations of these.}
\end{para}
...
However, since we do not require the Batanin tree representation for any other purpose, for brevity we simply give this single formulaic procedure, which suffices for the applications in the present work.
\end{para}
