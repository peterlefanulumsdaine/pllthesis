...
% \[\Delta: \E \two/->`<-/ \GSets : \Gamma,\] % Query: add pullback symbol?
...
\begin{para} \label{para:dtt-endo-operad}We saw above that for a type $A$ in any type theory $\T \in \DTT_\Id$, the contexts
...
\begin{lemma}The context $\x: \Gamma_\pi$, together with the obvious dependent projections, computes the limit $\Gamma_\pi = \lim_{c \in \int \pi} A_{\dim c}$.  Moreover, if $F \colon \T \to \S$ is a translation of type theories, then $\cl(F) \colon \T \to \cl(\S)$ preserves this limit.
\end{lemma}
\begin{proof}
...
\begin{theorem} \label{thm:main-thm-fundamental}Let $\T \in \DTT_\Id$ be any type theory with identity types, $A$ a closed type of $\T$.  Then the globular context $\A$ carries the structure of an internal $P_\Id$-algebra in $\T$.
\end{theorem}
