...
\section{Left and right maps in $\DTT$; the conservativity principle $\Jbar$}
...
For the remainder of this section, fix some collection $\Phi$ of the constructors and rules of Subsetcion \ref{subsec:construtors}, and work in $\DTT_\Phi$.  (The main cases of interest in the sequel are where $\Phi$ is either $(\Id)$, $(\Id,\Pi,\eta)$, or $(\Id,\Pi,\eta,\ext)$.)

For $n \geq 0$, we define theories $\T_\Phi[\Gamma_{(n)}]$, $\T_\Phi [\Gamma_{(n)} \types A]$, and $\T_\Phi [\Gamma_{(n)} \types a : A]$ to be the free theories on, respectively, a context of length $n$; a dependent type, in context of length $n$; and a term in such a type.  Axiomatically, each may be  specified by some subset of the rules below: $\T_\Phi[\Gamma_{(n)}]$ by the rules $i$-$\cxtrule$, for $0 \leq i < n$; $\T_\Phi [\Gamma_{(n)} \types A]$, by these rules together with $n$-$\typerule$; and $\T_\Phi [\Gamma_{(n)} \types a : A]$ by all of the above, together with $n$-$\termrule$:
...
$$ i^\ty_n \colon \T_\Phi [\Gamma_{(n)}] \mono \T_\Phi[\Gamma_{(n)} \types A],$$
$$ i^\tm_n \colon \T_\Phi [\Gamma_{(n)} \types A] \mono \T_\Phi[\Gamma_{(n)} \types a : A].$$
...
\para Once again, fix some set $\Phi$ of constructors, including at least the $\Id$-rules.  The \emph{type-theoretic globes} (over $\Phi$) are then a sequence of theories $\globe[n]$ which play a similar r\^o{}le in $\DTT_\Phi$ to that which the discs $D^n$ play in $\Top$: they are an internal weak-$\omega$-cocategory, and as such will---almost---be representing objects for the classifying weak $\omega$-category functor.
