...
\para As mentioned before, this is not quite what we want; $\cl^-_\omega(\T)$ only has types as objects, where we would like contexts.  To remedy the situation, we can compose with the ``dependent contexts'' endofunctor $(-)^\cxt \slice \diamond$ on $\DTT_\stuff$, and define $\cl(\T) := \cl^-_\omega(\T^\cxt \slice \diamond)$.
...
\para \label{para:map-from-cat} However, explicitly constructing the map gives us an excuse to analyse low dimensions of $\End(\globes)$.  A 0-dimensional operation in $\End(\globes)$ is perforce just a unary map $\globe[0] \to \globe[0]$ (there is only one 0-dimensional pasting diagram); so the single 0-dimensional operation in $\Cat$ (the identity on 0-cells) we send to the identity map $1_{\globe[0]}$.  (There is no choice: $\psi$ must preserve the operad structure, and $1_{\globe[0]}$ is the $0$-dimensional operad unit of $\End(\globes)$.)
...
The source and target maps of the right-hand cospan interpret the type of $\globe[0]$ as $A_0$ and $A_l$ respectively; so suitable maps $R : \globe[1] \to \T_\Phi[\path_l]$ correspond to interpretations of the type-constructor of $\globe[1]$ as some term
$$ x : A_0 \ \types\ t(x) : A_l $$
...
\para \label{para:canonicity-in-Tpath} When $\Phi$ gives a particularly well-behaved type system, we can say a little more.
...
\begin{proof}
Any failure of ``ext'' gives a failure of $\Jbar$.  \todo{[Expand on this!]}
\end{proof}
...
\begin{proof}
As seen in section \ref{sec:endo-operads}, contractibility for this operad demands that given any pasting diagram $\pi \in T1(n)$, and $(S_0,T_0,\ldots T_{n-1})$ as in Fig.\ \ref{fig:contractibility-pylons}, we must construct $R$ to complete the map of spans; more concisely, we must complete the triangle
...
First, we modify the spans we use to incorporate information about how the operations interact with the reflexivity retractions:

\begin{definition}
Let $\E$ be a category with pullbacks.  A \emph{higher endo-span with reflexivity data from dimension 1} in $\E$, or briefly for now a \emph{modified span}\footnote{The use of such a cumbersome name is intentional; this is clearly a rather ad hoc construction.  There are several modifications of spans we could consider here which would give the desired effect for the resulting operad; none seems particularly more natural than the others, and this one seems technically the simplest to work with. \todo{[Actually, there's an obvious more natural possibility---spans consisting of reflexive globular objects---and I think these might work; I just haven't checked through the full contractibility argument for them yet.]}}, consists of a higher span $(A_i,B_i,C,s_i,t_i,s_i',t_i')$, such that $A_i = B_i$ (and we also then write $A_n := C$), $s_i = s'_i$, and $t_i = t_i'$, together with maps $r_i \colon A_1 \to A_i$, for each $i \geq 1$, such that $s_i \cdot r_{i+1} = r_i = t_i \cdot r_{i+1}$, and $r_1 = 1_{A_1}$.  

A map of these consists of a map of spans $(f_i,g_i,h)$ as before, such that additionally $f_i \cdot r_i = r_i \cdot f_1$.  (We do \emph{not} insist that $f_i = g_i$!)

The compositions $\tensor_k$ of spans, and their corresponding unit spans, extend to modified spans, giving a monoidal globular category and forgetful functor:
$$\ModSpans[\E] \to \Spans[\E].$$
\end{definition}
...
\node A1(0,-1300)[A_1]
\node A0(0,-1700)[A_0]
\arrow|m|/@<-1ex>/[An`An1;s]
\arrow|m|/@<0ex>/[An`An1;t]
\arrow/@{}|<>(0.58)\vdots/[A2`An1;]
\arrow|m|/@<-1ex>/[A2`A1;s]
\arrow|m|/@<0ex>/[A2`A1;t]
\arrow|m|/@<-0.5ex>/[A1`A0;s]
\arrow|m|/@<0.5ex>/[A1`A0;t]
\arrow|m|/@/_0.5em//[A1`A2;r]
\arrow|m|/@/_1.35em//[A1`An1;r]
\arrow|m|/@/_2.5em//[A1`An;r]
\efig$$

\todo{Put this diagram in a sidebar to the definition, so it doesn't create so much whitespace.} The ``endo-span'' condition, that $A_i = B_i$ etc., says just that such a span may be seen as an $n$-globular object; this holds for all the spans appearing in the construction of any endomorphism operad.  We include it here to simplify the statement of the second condition, the reflexivity data.
...
\para In each of the above cases, the resulting map $L \to \End(\globes)$ allows us to pull the $\End(\globes)$-action on $\cl_\omega$ back to an action of $L$.  We thus have a functor into $\wkwCat$, which in flagrant abuse of notation we continue to denote $\cl_\omega$:
$$\bfig
\node DTT(0,0)[\DTT_\Phi]
\node EndGAlg(1000,500)[\Alg{\End(\globes)}]
\node GSets(1400,0)[\GSets]
