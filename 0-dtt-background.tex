...
\begin{para}[Structural core]
Since we will eventually work in a presentation-agnostic category of type theories, the precise formalism we use for the raw syntax will not be of importance; but for the sake of definiteness, let us suppose a simply-typed metalanguage in which the syntax is formalised, as described in e.g.\ \cite[6.1]{pitts:categorial-logic}.

The basic judgements of our theory are minimal---\emph{types}, \emph{terms}, and \emph{definitional equalities} between each of these:
\end{para}
...
On top of these, we work with derived judgements of \emph{contexts} and \emph{context morphisms} (also known as \emph{subtitutions} or \emph{telescopes}) between them:
\begin{center}\begin{tabular}{@{\ }c@{\hskip 0.75in}c@{\ }}
$\types \Gamma \ \cxt$ & $ \types {\vec f} : \Gamma \To \Gamma' $  \\ \rule{0pt}{3ex} 
$\types \Gamma = \Gamma' \ \cxt$ &  $\types {\vec f} = {\vec f}' : \Gamma \To \Gamma'$ \\
\end{tabular}
\end{center}
...
\begin{center}\begin{tabular}{@{\ }c@{\hskip 0.75in}c@{\ }}
$ \Gamma \types \Delta \ \cxt$ & $\Gamma \types {\vec f} : \Delta \To \Delta' $  \\ \rule{0pt}{3ex} 
$ \Gamma \types \Delta = \Delta' \ \cxt$ & $\Gamma \types {\vec f} = {\vec f}' : \Delta \To \Delta'$ \\
\end{tabular}
\end{center}

In slight abuse of notation, we will sometimes explicitly display (for emphasis or readability) free variables, writing e.g.\ $\x : \Gamma \types A(\x)\ \type$ interchangeable with $\Gamma \types A\ \type$.
