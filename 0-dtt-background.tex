...
{\Gamma\ \types\ J_{(A;\ x,y,u.\Delta(x,y,u);\ x,y,u,\w.C(x,y,u,\w))\,}(z,\v.\ d(z,\v);\ a,b,p,\c)\,: C(a,b,p,\c)}
\end{array}$$
Here all free variables are displayed, to emphasise the full formal binding that occurs in terms involving $J$; we will usually abbreviate the concluding term above to e.g.\ $J_C(d; a,b,p,\c)$, as for instance:
$$\begin{array}{c}
...
{\Gamma\ \types\ J_C(d;\ a,a,r(a),\c) = d(a) : C(a,a,r(a),\c)}
\end{array}$$

\todo{Discuss here or below the equivalence with the ``two-ended'' form.} \\
...
\begin{para}[Further rules for $\Id$- and $\Pi$-types]

\todo{To include: first, reflection, K, one-ended form of Id.  Then $\Pi$-$\eta$, $\Pi$-$\eta$-$\prop$, $\Pi$-$\ext$, $\Pi$-$\ext$-$\app$(-$\def$), and $\PiIdelim$.
