...
Decision, after discussion with Michael: $\C$ or $\Cbf$ probably least worst solution, or possibly $\mathbf{Cl}$?

\para Pasting diagrams: $\pd$, or $T1$, or $\mathbf{trees}$?  Probably $\pd$---now that I'm using B not L, $T1$ doesn't make sense, and trees are really imo just useful representations of them, not the fundamental objects themselves.  \todo{So go thru and do this!}
...
Decision:  Michael supports using $\del \pi$ for boundary.  

\para Consistency: $y(n)$ vs.\ $\yon(n)$ for cells!

\para Adjunctions? ---remind myself how to place the adjunction symbol them, in each of the various diagram styles I've used!  OK: done, now I know how to place them within an \verb|\efig|\ldots\verb|\bfig|; so, to do: \todo{do that throughout}.

\para Rules and terms: clearest current convention seems to be, use $\mathsf{sans serif}$ for constructors, including where they appear in rules (regrettably, thanks to inferrule's fetish for small caps, this seems to make it essential to use $\mathsf{\backslash mathsf}$ rather than \textsf{$\backslash$ textsf}), and small caps for other phrases appearing in rules?  \todo{Currently insonsistent; clean up!  In particular, check use of $\\Jrule$.}

\section{Terminologies}

\para Make sure to eradicate old $\CwA_\diamond$.

\para ``pylon diagrams'': fun but probably unnecessary; we already have ``higher spans'', which does the job fine. (Not to mention both ``telescopes'' and ``ladders'', which I abhor; going by which, others might well dislike ``pylon'' just as much!)  Michael agrees, no need for ``pylon''.

\para What term should we use for what $\globes$ form?  A co-weak $\omega$-category, a co-weak-$\omega$-category, a weak co-$\omega$-category, a weak $\omega$-cocategory?  A co-$P$-algebra, or a $P$-co-algebra?  Decision: weak $\omega$-cocategory, and $P$-coalgebra

\para Should I conscientiously talk about ``co-operations'' in ``co-endomorphism operads'', or is it clearer to drop the co-?  Michael agrees: yes!

\para $\omega$-groupoids, $\omega$-categories, $(\infty,1)$-categories?

\section{Checklist}  Notations etc.\ I should check for consistency during proofing:
\begin{itemize}
