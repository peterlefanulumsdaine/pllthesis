...
On top of these, we work with derived judgements of \emph{contexts} and \emph{context morphisms}\footnote{also sometimes known as \emph{substitutions} or \emph{telescopes}} between them:
\begin{center}\begin{tabular}{@{\ }c@{\hskip 0.75in}c@{\ }}
...
The above presentation of $\Id$-types via elim-structures is based on ideas of Gambino and Garner (\cite{gambino-garner}) which are implicit in much of the present work: that any CwA has important classes of left and right maps, which in the presence of $\Id$-types is moreover a weak factorisation system.  (These notions are defined and discussed in Section \ref{sec:wfs-bgd} below).
