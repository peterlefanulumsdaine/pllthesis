...
Matters are slightly subtler for the functional extensionality rules.  \todo{[Get a second opinion on this (RG has probably thought about it).]} $\Pi$-$\extrule$ alone does not seem to lift from $\C$ to $\C^\Cxt$; however, $\Pi$-$\extrule$ combined with either $\Pi$-$\extrule$-$\apprule$ or $\Pi$-$\extrule$-$\apprule$-$\defrule$ does lift.
