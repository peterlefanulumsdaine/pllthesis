...
\begin{para}[Type constructors] \label{para:constructors}
...
Two less destructive variations on the $\Id$-elimination rule are also worth mentioning; we will not use these as rules per se, but in Section \ref{sec:homot-strux-on-dtt} will discuss analogous variations on the principle $\Jbar$.
...
Contextual CwA's correspond precisely to the \emph{contextual categories} of Cartmell \cite{cartmell:generalised-algebraic-theories} and Streicher \cite{streicher:semantics-book}, and hence, crucially, to dependent type theories without constructors:

\begin{proposition}[John Cartmell \cite{cartmell:thesis},\cite{cartmell:generalised-algebraic-theories}] \label{prop:CwA-equivalence}
